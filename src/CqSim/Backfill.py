        ...
        while True:
            ...
            if not backfill_candidates:
                ...
                if sum_probabilities > 0:
                    probabilities = [float(i)/sum_probabilities for i in probabilities]
                    max_idx = np.random.choice(len(probabilities), p=probabilities)
                else:
                    # If all probabilities are zero, choose randomly from the candidates
                    if backfill_candidates:
                        max_idx = random.randint(0, len(backfill_candidates)-1)
                    else:
                        break # No candidates to choose from
