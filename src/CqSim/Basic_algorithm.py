...
class Basic_algorithm:
    def __init__ (self, carbon_intensity, ad_mode = 0, element = None, debug = None, para_list = None, ad_para_list=None, learning_model = None, start_date = None):
    ...
    def make_feature_vector(self, jobs, system_status, carbon_forecast):
        #print('self.model.job_cols',self.model.job_cols)
        job_cols = self.model.job_cols
        window_size = self.model.get_window_size()
        input_dim = [len(system_status)+window_size*job_cols + self.carbon_intensity.forecast_window_size, len(system_status[0])]
        ...
        offset = job_cols * window_size
        fv[0, offset:offset+len(system_status), :] = system_status
        offset += len(system_status)
        
        carbon_data = carbon_forecast[:self.carbon_intensity.forecast_window_size, :]
        fv[0, offset:offset+self.carbon_intensity.forecast_window_size, :] = carbon_data
        #print('fv',fv)
        return fv
    ...
    def compute_score_probabilities(self, wait_job, node_struc, currentTime, para_list = None, epsilon = 0.25, backfill = False):
        ...
        if (waitNum<=0):
            ...
            if self.start_date:
                ...
                current_real_time = start_datetime + timedelta(seconds=currentTime)
                carbon_forecast_input = self.carbon_intensity.forecast(current_date_time=current_real_time)
            else:
                raise ValueError("No start data found, cancelling run")
            ...
            feature_vectors = self.make_feature_vector(wait_job_input,system_status_input, carbon_forecast_input)
            probabilities = []
    ...
    def compute_score(self, wait_job, node_struc, currentTime, para_list = None, epsilon = 0.25, backfill = False):
        ...
        if (waitNum<=0):
            ...
            #pass
            if self.start_date:
                current_real_time = self.start_date + timedelta(seconds=currentTime)
                carbon_forecast_input = self.carbon_intensity.forecast(current_date_time=current_real_time)
            else:
                raise ValueError("No start data found, cancelling run")
            feature_vectors = None
