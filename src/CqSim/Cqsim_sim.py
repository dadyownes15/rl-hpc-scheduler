...
class Cqsim_sim:
    def __init__(self, module, start_date : datetime.datetime, carbon_intensity : CarbonIntensityGrid, debug = None, monitor = None, backfill_flag = 1, epsilon=0.25, epsilon_decay=0.9995,epsilon_min=0.001,training_size=10, lamda=0.7, discount_span=3600, weight_fn = None, weight_num=-1,reward_type='1',sleep='0',is_training='0'):
    ...
    def cqsim_sim(self):
        ...
        if self.weight_fn:
            if self.is_training!=0:
                ...
                print(f"................... Scanning for existing weights in: {checkpoint_folder}")
                print(f"................... Looking for files starting with: {checkpoint_files_start_with}")
                
                weight_files_found = []
                for file in os.listdir(checkpoint_folder):
                    if file.startswith(checkpoint_files_start_with) and file.endswith('.weights.h5'):
                        ...
                        file_str = file.replace(checkpoint_files_start_with,"").replace('.weights.h5','').rsplit('_',1)[-1]
                        if file_str.isdigit():
                            file_num = int(file_str)
                            lastest_num = max(lastest_num, file_num)
                            print(f"................... Found weight file: {file} (episode {file_num})")
        ...
        if self.is_training!=0 and self.weight_fn:
            ...
            print(f"................... Weight file: {self.weight_fn}_policy_{next_episode}.weights.h5")
       ...
       # self.debug.debug("# "+self.myInfo+" -- scan_event",5) 
        ...
        while (len(self.event_seq) > 0 or self.read_job_pointer >= 0):
            ...
            if (self.current_event['type'] == 1):
                ...
                self.debug.line(2,">>>") 
                self.debug.line(2,"--") 
                self.debug.debug("  Event: Job["+str(self.current_event['prio'])+"] "+str(self.currentTime),2)
                ...
                self.debug.line(2,"---") 
                self.debug.line(2,"--") 
                self.debug.debug("  Event: Monitor["+str(self.current_event['prio'])+"] "+str(self.currentTime),2)
                ...
                self.debug.line(2,"---") 
                self.debug.line(2,"--") 
                self.debug.debug("  Event: Extend["+str(self.current_event['prio'])+"] "+str(self.currentTime),2)
    ...
    def get_reward(self, job_info, max_job_wait_time):
        ...
        if self.reward_type == '2':
            ...
            job_start_dt = self.start_time + datetime.timedelta(seconds=int(self.currentTime)) 
            reserved_time = job_info['reqTime']
            reserved_processors = job_info['reqProc']
            carbon_reward = self.carbon_intensitiy.carbon_reward(job_start_time=job_start_dt, reserved_time=int(reserved_time), numProcess=int(reserved_processors), carbon_consideration_index=carbon_consideration_index)
            ...
            if self.training_cnt>=self.training_size:
                ...
                if self.reward_type in ('5', '7'):
                    cost = self.module['learning'].learn(
                        self.state_seq, self.action_seq, self.job_immediate_reward_seq)
                else:
                    cost = self.module['learning'].learn(
