...
class Cqsim_sim:
    def __init__(
        ...
        carbon_intensity: CarbonIntensityGrid,
        debug=None,
        monitor=None,
        backfill_flag=1,
        epsilon=0.25,
        ...
        self.carbon_intensitiy = carbon_intensity
        self.debug.line(4)
    ...
    def cqsim_sim(self):
        ...
        if self.weight_fn:
            if self.is_training != 0:
                checkpoint_folder, checkpoint_files_start_with = self.weight_fn.rsplit(
                    "/", 1
                )
                ...
                print(
                    f"................... Scanning for existing weights in: {checkpoint_folder}"
                )
                print(
                    f"................... Looking for files starting with: {checkpoint_files_start_with}"
                )

                weight_files_found = []
                for file in os.listdir(checkpoint_folder):
                    if file.startswith(checkpoint_files_start_with) and file.endswith(
                        ".weights.h5"
                    ):
                        weight_files_found.append(file)
                        # Extract number from filename like "dras_theta_policy_3.weights.h5"
                        # Remove the base name and extension, then get the last part after splitting by '_'
                        file_str = (
                            file.replace(checkpoint_files_start_with, "")
                            .replace(".weights.h5", "")
                            .rsplit("_", 1)[-1]
                        )
                        if file_str.isdigit():
                            ...
                            print(
                                f"................... Found weight file: {file} (episode {file_num})"
                            )

                if weight_files_found:
                    print(f"................... Latest episode found: {lastest_num}")
                else:
                    print(
                        "................... No existing weight files found, starting from episode 0"
                    )

                policy_filename = (
                    self.weight_fn + "_policy_" + str(lastest_num) + ".weights.h5"
                )

                if os.path.exists(policy_filename):
                    self.module["learning"].load_weights(self.weight_fn, lastest_num)
                    print(
                        f"................... Successfully loaded weights from episode {lastest_num}"
                    )
                    print(f"................... Policy file: {policy_filename}")
                else:
                    print(
                        f"................... Weight file for episode {lastest_num} not found, starting fresh"
                    )
            else:
                ...
                print(
                    f"................... Non-training mode: Using weight_num={self.weight_num}"
                )
                print(f"................... Expected policy file: {policy_filename}")

                if os.path.exists(policy_filename):
                    self.module["learning"].load_weights(
                        self.weight_fn, self.weight_num
                    )
                else:
                    print(
                        "................... Weight file not found, running with random policy"
                    )
        ...
        self.import_submit_events()  # Gets new events and inserts them into the queue
        # self.insert_event_job()
        self.insert_event_extend()
        self.scan_event()

        if self.is_training != 0 and self.weight_fn:
            next_episode = lastest_num + 1
            self.module["learning"].save_weights(self.weight_fn, next_episode)
            print(f"................... Saving weights for episode {next_episode}")
            print(
                f"................... Weight file: {self.weight_fn}_policy_{next_episode}.weights.h5"
            )
    ...
    def scan_event(self):
        ...
        # Main simulation loop: continues as long as there are events in the queue or jobs to be read
        while len(self.event_seq) > 0 or self.read_job_pointer >= 0:
            # print('event_seq',len(self.event_seq))

            # Block 1: Get the next event from the queue
            ...
            # Block 2: Dynamically load more jobs if necessary
            ...
            # Block 3: Set current event and time
            self.current_event = temp_current_event
            self.currentTime = temp_currentTime

            # Block 4: Process the current event based on its type
            ...
            # Block 5: Collect system statistics after processing the event
            self.sys_collect()
    ...
    def score_calculate(self, wait_jobs, backfill=False):
        ...
        while i < wait_num:
            ...
            # idx_list.append(i)
            max_wait_time = max(
                max_wait_time, self.currentTime - temp_job["submit"]
            )  # all jobs or eligible jobs ?
            i += 1
        ...
        if backfill:
            max_idx, selected_fv = self.module["alg"].compute_score(
                temp_wait,
                temp_nodeStruc,
                self.currentTime,
            ...
            time1 = time.time()
            probabilities, selected_fv = self.module["alg"].compute_score_probabilities(
                temp_wait, temp_nodeStruc, self.currentTime, epsilon=self.epsilon
            )
            ...
            prob = probabilities[max_idx] if probabilities else None
        ...
        return max_idx, selected_fv, max_wait_time, prob

    def update_job_order(self, wait_jobs):
        # Block 1: Use the RL model to select the best job to run next
        max_idx, selected_fv, max_job_wait_time, prob = self.score_calculate(wait_jobs)

        # Block 2: Reorder the wait queue to move the selected job to the front
        ...
        if max_idx > 0 and max_idx < len(wait_jobs):
            ...
            temp_wait_jobs = (
                [wait_jobs[max_idx]] + wait_jobs[:max_idx] + wait_jobs[max_idx + 1 :]
            )
            return temp_wait_jobs, selected_fv, max_job_wait_time, max_idx, prob
        else:
            # If the best job is already at the front or the index is invalid, keep the original order
            return wait_jobs, selected_fv, max_job_wait_time, max_idx, prob

    def keep_job_order(self, wait_jobs):
        max_idx, selected_fv, max_job_wait_time, _ = self.score_calculate(
            wait_jobs, backfill=True
        )
        return wait_jobs, selected_fv, max_job_wait_time, max_idx, None
    ...
    def get_reward(self, job_info, max_job_wait_time):
        ...
        if self.reward_type == "2":
            ...
            self.reward_dict[self.currentTime] = sum(
                self.wait_times_dict[self.currentTime]
            ) / len(self.wait_times_dict[self.currentTime])
            ...
            cur_average_wait_time_ratio = sum(
                self.wait_times_dict[self.currentTime]
            ) / len(
                self.wait_times_dict[self.currentTime]
            )  # average or minimimal ?
            self.reward_dict[self.currentTime] = (
                max(self.uti_dict[self.currentTime], uti) + cur_average_wait_time_ratio
            ) / 2
            ...
            if self.currentTime in self.uti_dict:
                if job_info["award"] == 1:
                    uti = (
                        self.uti_dict[self.currentTime]
                        + float(job_info["reqProc"] * priority_weigth) / tot
                    )
                else:
                    uti = (
                        self.uti_dict[self.currentTime]
                        + float(job_info["reqProc"]) / tot
                    )
            else:
                idle = self.module["node"].get_idle()
                uti = float(tot - idle) / tot
            ...
            carbon_consideration_index = job_info["award"]
            assert isinstance(self.start_time, datetime.datetime)
            job_start_dt = self.start_time + datetime.timedelta(
                seconds=int(self.currentTime)
            )
            reserved_time = job_info["reqTime"]
            reserved_processors = job_info["reqProc"]
            carbon_reward = self.carbon_intensitiy.carbon_reward(
                job_start_time=job_start_dt,
                reserved_time=int(reserved_time),
                numProcess=int(reserved_processors),
                carbon_consideration_index=carbon_consideration_index,
            )

            tmp_reward += carbon_reward * w4
    ...
    def start_scan(self):
        ...
        # Block 1: Main scheduling loop to start jobs from the wait queue
        while True:
            ...
            # Block 2: Determine job order and collect training data
            # print('temp_wait_before',temp_wait)
            max_idx = self.module["learning"].window_size
            if temp_wait[0] == self.reserve_job_id:
                # Keep the current job order if the top job is reserved
                temp_wait, selected_fv, max_job_wait_time, max_idx, _ = (
                    self.keep_job_order(temp_wait)
                )
            else:
                # Update job order using the RL model and get reward for the decision
                temp_wait, selected_fv, max_job_wait_time, max_idx, prob = (
                    self.update_job_order(temp_wait)
                )
                ...
                if self.module["learning"].algorithm == "ppo":
                    self.action_prob_seq.append(prob)

            if max_idx < 0:
                max_idx = self.module["learning"].window_size

            # Block 3: Attempt to start the highest priority job
            ...
            # Block 4: If the top job cannot start, attempt backfilling
            else:
                temp_wait = self.module["job"].wait_list()
                if len(temp_wait) >= 1:
                    # Reserve the top job
                ...
                if self.backfill_flag > 0 and len(temp_wait) > 1:
                    # Start backfilling with the rest of the queue
                    self.backfill(temp_wait)
                break

        # Block 5: Perform batch training if the training size is reached (only during training)
        if self.is_training != 0:
            self.training_cnt += 1
            if self.training_cnt >= self.training_size:
                ...
                # For reward types that rely on immediate, per-decision rewards, use
                # job_immediate_reward_seq.  These are currently types 5 and 7.
                if self.reward_type in ("5", "7"):
                    rewards = self.job_immediate_reward_seq
                else:
                    rewards = self.reward_seq
                ...
                # Reset training parameters for the next batch
                self.training_cnt = 0
                self.value_seq = []
                self.grads_seq = []
                self.reward_seq = []
                ...
                self.debug.debug(
                    "Batch Training Time: " + str(time.time() - start_train_time), 3
                )
    ...
    def backfill(self, temp_wait):
            ...
            temp_wait_info.append(
                {
                    ...
                    "run": temp_job["run"],
                    "score": temp_job["score"],
                    "submit": temp_job["submit"],
                    "reqTime": temp_job["reqTime"],
                    "reqProc": temp_job["reqProc"],
                    "award": temp_job["award"],
                }
            )
            i += 1
        # backfill_list = self.module['backfill'].backfill(temp_wait_info, {'time':self.currentTime})
        time1 = time.time()
        backfill_list, selected_fv_list, max_idxs = self.module["backfill"].backfill(
            temp_wait_info,
            {
                "time": self.currentTime,
                "alg_module": self.module["alg"],
                "epsilon": self.epsilon,
            },
        )
        ...
        if selected_fv_list:
            for job_index, selected_fv, max_idx in zip(
                backfill_list, selected_fv_list, max_idxs
            ):
                if job_index >= 0:
    ...
    def sys_collect(self):
        ...
        temp_info = self.module["info"].info_collect(
            ...
            * 1.0
            ...
            waitCore=self.module["job"].wait_core_seconds(),
            inter=temp_inter,
        )
