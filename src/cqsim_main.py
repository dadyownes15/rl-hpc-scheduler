...
def  cqsim_main(para_list):
    ...
    if unix_start_time:
        ...
        para_list['start_date'] = datetime.fromtimestamp(unix_start_time, tz=timezone.utc)
    #    print(f"  [INFO] 'start_date' set from SWF header (UTC): {para_list['start_date']}")
        ...
        print(f"  [WARN] 'UnixStartTime' not found in SWF header. 'start_date' will be None.")
    ...
    carbon_itensity = CarbonIntensityGrid.CarbonIntensityGrid(forecast_type=para_list['forecast_mode'], forecast_window_size = para_list['carbon_forecast_length'])
    # Job Filter
    print(".................... Job Filter")
    ...
    input_dim = [node_size + window_size * job_cols + carbon_forecast_length, int(para_list['input_dim'])]
    ...
    value_model = Class_Reinforcement_learning.ValueModel(debug=module_debug, input_dim=input_dim, job_cols=job_cols, window_size = window_size, hidden_dim_str = para_list['hidden_dim'], node_module=module_node_struc, GAMMA=para_list['lamda'])

    # Basic Algorithm
    print(".................... Basic Algorithm")
    module_alg = Class_Basic_algorithm.Basic_algorithm (element=[para_list['alg'],para_list['alg_sign']],debug=module_debug,para_list=para_list['ad_alg_para'], learning_model = value_model, start_date=para_list['start_date'], carbon_intensity=carbon_itensity)
    ...
    module_sim = Class_Cqsim_sim.Cqsim_sim(module=module_list, start_date=para_list['start_date'], carbon_intensity= carbon_itensity, debug=module_debug, monitor = para_list['monitor'], backfill_flag=para_list['bf_flag'], epsilon=para_list['epsilon'], epsilon_decay=para_list['epsilon_decay'], epsilon_min=para_list['epsilon_min'], training_size=para_list['training_size'],lamda=para_list['lamda'],discount_span=para_list['discount_span'], weight_fn = weight_fn, weight_num=para_list['weight_num'],reward_type =para_list['reward_type'], sleep =para_list['sleep'], is_training=para_list['is_training'])
